...
func main() {
	...
	for _, pass := range input {
		...
		count := strings.Count(pass.password, string(pass.char))
		if count >= pass.min && count <= pass.max {
			counter++
		}
		charArr := []rune(pass.password)

		check := len(charArr) >= pass.min && charArr[pass.min-1] == pass.char

		if secondcheck := len(charArr) >= pass.max && charArr[pass.max-1] == pass.char; secondcheck {
